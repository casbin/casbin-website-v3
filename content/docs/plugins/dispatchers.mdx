...
<Callout>
  Dispatchers synchronize runtime policy changes only. They do not resolve pre-existing
  inconsistencies during initialization. You must ensure all enforcer instances start with identical
  policy states before enabling dispatchers.
</Callout>
