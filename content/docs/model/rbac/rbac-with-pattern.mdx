...
![editor-tips](/images/editor-tips.png)
...
<Callout>
  Pattern matching with domains supports custom domain token names. Instead of `dom`, you can use
  any identifier like `tenant` or `workspace`. Casbin automatically detects the domain token name
  from your model definition when using pattern matching functions like `keyMatch`.
</Callout>
