...
<Callout type="warn">
  **Important:** ABAC attribute access works only for request elements (r.sub, r.obj, r.act). Policy
  elements like p.sub cannot use ABAC since policies cannot store struct or class definitions.
</Callout>

<Callout type="info">
  Matchers can reference multiple attributes simultaneously. Example: `m = r.sub.Domain ==
  r.obj.Domain`.
</Callout>

<Callout type="info">
  When policies require commas that would conflict with CSV parsing, wrap the expression in
  quotation marks. Example: `"keyMatch("bob", r.sub.Role)"` prevents unwanted splitting.
</Callout>
