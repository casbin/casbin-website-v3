...
<Callout>
  When handling domain names that contain `::`, unexpected behavior may occur. In Casbin, `::` is a
  reserved keyword, similar to `for` or `if` in programming languages. Never use `::` within a
  domain name.
</Callout>
...
<Callout>
  This method does not apply to domains that have an inheritance relationship, also known as
  implicit roles.
</Callout>
...
<Callout>Only users will be returned, roles (2nd arg in "g") will be excluded.</Callout>
